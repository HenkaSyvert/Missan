...
void StandardMap() {
    ...
    GameObject player;                              // The GameObject holding the camera
    player.AddComponent<Camera>();                  // The actual Camera, through which we can view the world
    player.AddComponent<FPSCamera>();               // our custom-made script to make it behave like a flying FPS style camera
    transform = player.GetComponent<Transform>();
    transform->rotationDeg = { -15,180,0 };
    transform->position = { 0,2,-10 };
    player.AddComponent<Collider>();                // so we know if we collide into things
    ...
    GameObject* playerGo = Engine::Instantiate(player);
    Graphics::SetCamera(*playerGo->GetComponent<Camera>()); 
    playerGo->GetComponent<FPSCamera>()->moveCam = &playerGo->GetComponent<Menu>()->canMoveCam;
